...
$convId = (int)($in['id'] ?? 0);
$draft = trim((string)($in['text'] ?? ''));
if ($draft === '') json_out(['ok' => false, 'error' => 'réponse vide'], 400);
...
$ai = ai_improve_reply($property, $conv, $recent, $draft);
...
if (!empty($ai['error'])) {
    history_add([
        ...
        'body' => $draft,
        'actor' => 'admin',
        'error_text' => 'Amélioration IA : ' . $ai['error'],
    ]);
    json_out(['ok' => false, 'error' => $ai['error'], 'reason' => $ai['reason'] ?? ''], 502);
}

$reason = $ai['requires_validation'] ? ($ai['reason'] ?: 'Validation conseillée') : 'Réponse améliorée par IA';
